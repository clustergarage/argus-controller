...
const (
	...
	// ErrResourceExists is used as part of the Event 'reason' when a FimWatcher fails
	// to sync due to a Deployment of the same name already existing.
	ErrResourceExists = "ErrResourceExists"

	// MessageResourceExists is the message used for Events when a resource
	// fails to sync due to a Deployment already existing
	MessageResourceExists = "Resource %q already exists and is not managed by FimWatcher"
...
type Controller struct {
	...
	fimWatchersLister listers.FimWatcherLister
	fimWatchersSynced cache.InformerSynced
...
func NewController(
	...
	fimWatcherInformer informers.FimWatcherInformer) *Controller {
	...
	controller := &Controller{
		...
		fimWatchersLister: fimWatcherInformer.Lister(),
		fimWatchersSynced: fimWatcherInformer.Informer().HasSynced,
	...
	fimWatcherInformer.Informer().AddEventHandler(cache.ResourceEventHandlerFuncs{
		...
		UpdateFunc: func(old, new interface{}) {
			controller.enqueueFimWatcher(new)
		},
	})
	// Set up an event handler for when Deployment resources change. This
	// handler will lookup the owner of the given Deployment, and if it is
	// owned by a FimWatcher resource will enqueue that FimWatcher resource for
...
func (c *Controller) syncHandler(key string) error {
	...
	fimWatcher, err := c.fimWatchersLister.FimWatchers(namespace).Get(name)
	if err != nil {
	...
	deploymentName := fimWatcher.Spec.DeploymentName
	if deploymentName == "" {
	...
	// Get the deployment with the name specified in FimWatcher.spec
	deployment, err := c.deploymentsLister.Deployments(fimWatcher.Namespace).Get(deploymentName)
	// If the resource doesn't exist, we'll create it
	if errors.IsNotFound(err) {
		deployment, err = c.kubeclientset.AppsV1().Deployments(fimWatcher.Namespace).Create(newDeployment(fimWatcher))
	}
	...
	// If the Deployment is not controlled by this FimWatcher resource, we should log
	// a warning to the event recorder and ret
	if !metav1.IsControlledBy(deployment, fimWatcher) {
		msg := fmt.Sprintf(MessageResourceExists, deployment.Name)
		c.recorder.Event(fimWatcher, corev1.EventTypeWarning, ErrResourceExists, msg)
		return fmt.Errorf(msg)
	}

	// If this number of the replicas on the FimWatcher resource is specified, and the
	...
	// Finally, we update the status block of the FimWatcher resource to reflect the
	// current state of the world
	err = c.updateFimWatcherStatus(fimWatcher, deployment)
	if err != nil {
		return err
	}

	c.recorder.Event(fimWatcher, corev1.EventTypeNormal, SuccessSynced, MessageResourceSynced)
	return nil
}

func (c *Controller) updateFimWatcherStatus(fimWatcher *fimv1alpha1.FimWatcher, deployment *appsv1.Deployment) error {
	// NEVER modify objects from the store. It's a read-only, local cache.
	// You can use DeepCopy() to make a deep copy of original object and modify this copy
	// Or create a copy manually for better performance
	fimWatcherCopy := fimWatcher.DeepCopy()
	fimWatcherCopy.Status.AvailableReplicas = deployment.Status.AvailableReplicas
	...
	_, err := c.fimclientset.FimcontrollerV1alpha1().FimWatchers(fimWatcher.Namespace).Update(fimWatcherCopy)
	return err
}
...
func (c *Controller) enqueueFimWatcher(obj interface{}) {
	var key string
	var err error
...
// to find the FimWatcher resource that 'owns' it. It does this by looking at the
// objects metadata.ownerReferences field for an appropriate OwnerReference.
// It then enqueues that FimWatcher resource to be processed. If the object does not
...
func (c *Controller) handleObject(obj interface{}) {
	...
	if ownerRef := metav1.GetControllerOf(object); ownerRef != nil {
		...
		c.enqueueFimWatcher(fimWatcher)
		return
	}
}

// newDeployment creates a new Deployment for a FimWatcher resource. It also sets
// the appropriate OwnerReferences on the resource so handleObject can discover
// the FimWatcher resource that 'owns' it.
func newDeployment(fimWatcher *fimv1alpha1.FimWatcher) *appsv1.Deployment {
	labels := map[string]string{
		"app":        "nginx",
		"controller": fimWatcher.Name,
	}
	return &appsv1.Deployment{
		ObjectMeta: metav1.ObjectMeta{
			Name:      fimWatcher.Spec.DeploymentName,
			Namespace: fimWatcher.Namespace,
			OwnerReferences: []metav1.OwnerReference{
				*metav1.NewControllerRef(fimWatcher, schema.GroupVersionKind{
		...
		Spec: appsv1.DeploymentSpec{
			Replicas: fimWatcher.Spec.Replicas,
